...
%         switch_steering =   100;  % sswitch from fixed steering to PI,    [s]
        ...
        switch_speed_control  = 100;  
                                % time when speed PI receives setpoint, [s]    
        max_speed       =   80;  % kph 
        lat_xmin        = -22;  % min x value, position plot, [m]
        lat_ymin        =  -1;  % min y value, position plot, [m]
        lat_xmax        =   1; % max x value, position plot,  [m]
        lat_ymax        =   22; % max y value, position plot,  [m]
...
%         switch_steering =   3.58;  % switch from fixed steering to PI,    [s]
        switch_steering =  100;  % switch from fixed steering to PI,   [s]
        steering_step_t =  0.01;  % steering step time to induce drift, [s]
        steering_step   =  -15;  % steering angle to induce drift
...
%         switch_speed_control  = 100;  
                                % time when speed PI receives setpoint, [s]                           
        max_speed       =   300;  % kph 
        lat_xmin        = -26;  % min x value, position plot, [m]
        lat_ymin        =  -1;  % min y value, position plot, [m]
        lat_xmax        =   1; % max x value, position plot,  [m]
        lat_ymax        =   26; % max y value, position plot,  [m]
        latv_setpoint   =   -8; % lateral velocity setpoint,   [kph]
...
%         switch_speed_control  = 100;  
                                % time when speed PI receives setpoint, [s]                           
        max_speed       =   80;  % kph 
        lat_xmin        = -16;  % min x value, position plot, [m]
        lat_ymin        =  -1;  % min y value, position plot, [m]
        lat_xmax        =   1; % max x value, position plot,  [m]
        lat_ymax        =   16; % max y value, position plot,  [m]
        latv_setpoint   =   -12.28; % lateral velocity setpoint,   [kph]
...
% save('MCout_throttle_control_8','MCout','simout')
% % 
% 
% %% if desired, uncomment to plot results
% figure
% scatter3(MCout(1,:),MCout(2,:),abs(MCout(3,:)),3,MCout(4,:))
% xlabel('steering angle [deg]')
% ylabel('real wheel speed [rpm]')
% zlabel('lateral (drift) speed [|kph|]')
% colormap(jet)
% h = colorbar;
